...
A YAML that fails to parse or validate aborts the hook: the error is written to stderr (one line, `[tools-runner] failed to load <displayFile>: <message>`) and to the audit log as a `hook_error` entry, and the hook exits 2 without running any commands. Exit 2 is the documented Claude Code Stop-hook signal that means "blocking error": the stderr line is fed back to the model on the next turn so the failure surfaces inside the conversation rather than only in the transcript. Aborting on any malformed layer is intentional: a layer silently treated as empty would mean the user's tests stop running with no obvious cause, which is worse than a noisy failure they can fix. A file that is missing at load time, by contrast, is observationally identical to a file with `triggers: []` (the layer simply contributes no triggers).
...
- `hook_error`: written when the top-level `try/catch` catches an unhandled exception. May be followed by `hook_completed` with `exitCode: 2`, or `process.exit(2)` may fire first; the absence of a paired `hook_completed` is itself a signal.
